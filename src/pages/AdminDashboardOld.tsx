...
import type { Page, Service, Specialization, Client, Banner } from '../types/database';
import { Trash2, Edit, Plus, Save, X, Upload, Image, Layout, Layers, Users, Monitor, Briefcase } from 'lucide-react';
...
export default function AdminDashboard({ onSettingsUpdate }: AdminDashboardProps) {
  ...
  const renderSidebar = () => (
      <div className="md:col-span-1 space-y-2">
          <button onClick={() => setActiveTab('pages')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-bold transition-all ${activeTab === 'pages' ? 'bg-[#ec533a] text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
              <Layout className="h-5 w-5" /> <span>الصفحات (الأقسام)</span>
          </button>
          <button onClick={() => setActiveTab('specializations')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-bold transition-all ${activeTab === 'specializations' ? 'bg-[#ec533a] text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
              <Layers className="h-5 w-5" /> <span>التخصصات والعملاء</span>
          </button>
          <button onClick={() => setActiveTab('banners')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg font-bold transition-all ${activeTab === 'banners' ? 'bg-[#ec533a] text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
  ...
  return (
    <div className="min-h-screen font-[Cairo] bg-[#162341] text-white" dir="rtl">
        ...
        <div className="bg-black/60 backdrop-blur-md border-b border-white/10 sticky top-0 z-40">
            <div className="container mx-auto px-4 py-4 flex justify-between items-center">
                <h1 className="text-2xl font-bold text-[#ec533a]">لوحة التحكم - POVA</h1>
            ...
            <div className="md:col-span-3">
                ...
                {activeTab === 'pages' && (
                    <div className="space-y-8">
                        <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-6">
                            <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Layout className="text-[#ec533a]" /> {editingPage ? 'تعديل صفحة' : 'إضافة صفحة جديدة'}</h2>
                            <form onSubmit={editingPage ? handleUpdatePage : handleAddPage} className="space-y-4">
                                <input type="text" placeholder="اسم الصفحة (مثال: خدماتنا، من نحن)" value={newPage.name} onChange={e => setNewPage({...newPage, name: e.target.value})} className="w-full p-3 bg-gray-700 border border-gray-600 rounded focus:border-[#ec533a] focus:outline-none" />
                                <input type="text" placeholder="Slug (رابط الصفحة - اختياري)" value={newPage.slug || ''} onChange={e => setNewPage({...newPage, slug: e.target.value})} className="w-full p-3 bg-gray-700 border border-gray-600 rounded focus:border-[#ec533a] focus:outline-none" dir="ltr" />
                                <textarea placeholder="وصف الصفحة" value={newPage.description || ''} onChange={e => setNewPage({...newPage, description: e.target.value})} className="w-full p-3 bg-gray-700 border border-gray-600 rounded focus:border-[#ec533a] focus:outline-none" rows={3} />
                                <div className="flex gap-2">
                                    <button type="submit" className="bg-[#ec533a] hover:bg-[#d63d2a] text-white px-6 py-2 rounded font-bold flex-1">{editingPage ? 'حفظ التعديلات' : 'إضافة الصفحة'}</button>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                            {pages.map(page => (
                                <div key={page.id} className="bg-gray-800 border border-gray-700 p-4 rounded-lg flex justify-between items-center group hover:border-[#ec533a] transition-colors">
                                    <div>
                ...
                {activeTab === 'specializations' && (
                    <div className="space-y-8">
                        ...
                        <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-6">
                            <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Layers className="text-[#ec533a]" /> {editingSpecialization ? 'تعديل تخصص' : 'إضافة تخصص جديد'}</h2>
                            <form onSubmit={editingSpecialization ? handleUpdateSpecialization : handleAddSpecialization} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <select value={newSpecialization.page_id} onChange={e => setNewSpecialization({...newSpecialization, page_id: e.target.value})} className="w-full p-3 bg-gray-700 border border-gray-600 rounded focus:border-[#ec533a] focus:outline-none">
                        ...
                        <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-6 border-t-4 border-t-[#ec533a]">
                             <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Users className="text-[#ec533a]" /> {editingClient ? 'تعديل بيانات عميل' : 'إضافة عميل جديد'}</h2>
                             <form onSubmit={editingClient ? handleUpdateClient : handleAddClient} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <select value={newClient.specialization_id} onChange={e => setNewClient({...newClient, specialization_id: e.target.value})} className="w-full p-3 bg-gray-700 border border-gray-600 rounded focus:border-[#ec533a] focus:outline-none">
                                        <option value="">اختر التخصص التابع له</option>
                                        {specializations.map(s => <option key={s.id} value={s.id}>{s.name_ar}</option>)}
                                    </select>
                                    <input type="text" placeholder="اسم العميل / المشروع" value={newClient.name} onChange={e => setNewClient({...newClient, name: e.target.value})} className="w-full p-3 bg-gray-700 border border-gray-600 rounded focus:border-[#ec533a] focus:outline-none" />
                                </div>
                                <div className="flex gap-4 items-center">
                                     <div className="flex-1">
                                         <label className="block text-sm text-gray-400 mb-1">شعار العميل / صورة المشروع</label>
                                         <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, 'client')} className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#ec533a] file:text-white hover:file:bg-[#d63d2a]" disabled={uploadingImage} />
                                     </div>
                                     {newClient.logo_url && <img src={newClient.logo_url} alt="Logo" className="w-16 h-16 object-cover rounded bg-white" />}
                                </div>
                                <textarea placeholder="وصف العميل / تفاصيل المشروع" value={newClient.description || ''} onChange={e => setNewClient({...newClient, description: e.target.value})} className="w-full p-3 bg-gray-700 border border-gray-600 rounded focus:border-[#ec533a] focus:outline-none" rows={3} />
                                <div className="flex gap-2">
                                    <button type="submit" className="bg-[#ec533a] hover:bg-[#d63d2a] text-white px-6 py-2 rounded font-bold flex-1" disabled={uploadingImage}>{editingClient ? 'حفظ التعديلات' : 'إضافة العميل'}</button>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {clients.map(client => (
                                <div key={client.id} className="bg-gray-800 border border-gray-700 p-4 rounded-lg flex gap-4 group hover:border-[#ec533a] transition-all">
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-[#ec533a] mb-1">{client.specialization?.name_ar}</p>
                ...
                {activeTab === 'banners' && (
                     <div className="space-y-8">
                        <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-6">
                            <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Image className="text-[#ec533a]" /> {editingBanner ? 'تعديل بانر' : 'إضافة بانر جديد'}</h2>
                            <form onSubmit={editingBanner ? handleUpdateBanner : handleAddBanner} className="space-y-4">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                     <input type="text" placeholder="عنوان البانر (اختياري)" value={newBanner.title || ''} onChange={e => setNewBanner({...newBanner, title: e.target.value})} className="w-full p-3 bg-gray-700 border border-gray-600 rounded focus:border-[#ec533a] focus:outline-none" />
                                     <select value={newBanner.page_id || ''} onChange={e => setNewBanner({...newBanner, page_id: e.target.value || null})} className="w-full p-3 bg-gray-700 border border-gray-600 rounded focus:border-[#ec533a] focus:outline-none">
                                ...
                                <div>
                                     ...
                                     <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, 'banner')} className="block w-full text-sm text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-[#ec533a] file:text-white hover:file:bg-[#d63d2a]" disabled={uploadingBannerImage} />
                                ...
                                <div className="flex gap-2">
                                    <button type="submit" className="bg-[#ec533a] hover:bg-[#d63d2a] text-white px-6 py-2 rounded font-bold flex-1" disabled={uploadingBannerImage}>{editingBanner ? 'حفظ التعديلات' : 'إضافة البانر'}</button>
                        ...
                        <div className="grid grid-cols-1 gap-4">
                             {banners.map(banner => (
                                <div key={banner.id} className="bg-gray-800 border border-gray-700 p-4 rounded-lg group hover:border-[#ec533a] transition-colors">
