...
export default function AdminDashboard({ onSettingsUpdate }: AdminDashboardProps) {
  ...
  const handleEditClick = (e: React.MouseEvent, linkedPage: Page | null, item: ReturnType<typeof resolveCoreServicesWithPages>[number]) => {
    e.stopPropagation();
    if (linkedPage) {
      ...
      <div
        ...
        onClick={() => {
          if (linkedPage) {
            ...
            toast.info('هذه الخدمة غير مربوطة بقاعدة البيانات. يمكنك ربطها من زر التعديل.');
          }
        }}
      >
        ...
        <div className="pointer-events-none relative z-10 flex h-full flex-col p-6">
          ...
          <button
            onClick={(e) => handleEditClick(e, linkedPage, item)}
