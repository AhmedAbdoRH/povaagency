...
const emptyPageForm = { name: '', description: '', image_url: '', banner_url: '' };
const emptySpecForm = { service_id: '', name: '', description: '', image_url: '' };
const emptyClientForm = { specialization_id: '', name: '', description: '', image_url: '', project_url: '', logo_url: '' };
        ...
        <button
          onClick={() => {
            if (linkedPage) {
              ...
              setEditingPage(null);
              setPageForm({ name: item.title, description: item.description, image_url: '', banner_url: '' });
              setActiveForm('page');
            }
          }}
          ...
          <button
            onClick={() => {
              if (linkedPage) {
                ...
                setPageForm({ name: linkedPage.name, description: linkedPage.description || '', image_url: linkedPage.image_url || '', banner_url: linkedPage.banner_url || '' });
              } else {
                setEditingPage(null);
                setPageForm({ name: item.title, description: item.description, image_url: '', banner_url: '' });
              }
        ...
        <div className="rounded-3xl border border-gray-700/50 bg-gray-800/50 p-6">
          ...
          {selectedSpec && !selectedClient && (
            <div>
              ...
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">{filteredClients.map(client => <div key={client.id} className="overflow-hidden rounded-2xl border border-gray-600/50 bg-gray-700/30"><button onClick={() => setSelectedClient(client.id)} className="w-full text-right"><div className="relative aspect-square">{client.image_url ? <img src={client.image_url} alt={client.name} className="h-full w-full object-cover" /> : <div className="h-full w-full bg-gray-800/50" />}<div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent" /><div className="absolute bottom-0 left-0 right-0 p-3"><h3 className="line-clamp-2 font-bold">{client.name}</h3></div></div></button><div className="flex gap-2 p-3"><button onClick={() => { setEditingClient(client.id); setClientForm({ specialization_id: client.specialization_id, name: client.name, description: client.description || '', image_url: client.image_url || '', project_url: client.project_url || '', logo_url: client.logo_url || '' }); setActiveForm('client'); }} className="flex-1 rounded-lg bg-green-500/20 py-2 text-green-200"><Edit className="mx-auto h-4 w-4" /></button><button onClick={() => removeItem('clients', client.id)} className="flex-1 rounded-lg bg-red-500/20 py-2 text-red-200"><Trash2 className="mx-auto h-4 w-4" /></button></div></div>)}</div>
              {activeForm === 'client' && <form onSubmit={saveClient} className="mt-6 grid gap-4 rounded-2xl border border-gray-600/50 bg-gray-700/30 p-5"><input value={clientForm.name} onChange={e => setClientForm({ ...clientForm, name: e.target.value })} placeholder="اسم العمل أو العميل" className="rounded-xl bg-gray-800/50 p-4" required /><textarea value={clientForm.description} onChange={e => setClientForm({ ...clientForm, description: e.target.value })} placeholder="وصف العمل" rows={3} className="rounded-xl bg-gray-800/50 p-4" /><input value={clientForm.project_url} onChange={e => setClientForm({ ...clientForm, project_url: e.target.value })} placeholder="رابط المشروع" className="rounded-xl bg-gray-800/50 p-4" /><input value={clientForm.logo_url} onChange={e => setClientForm({ ...clientForm, logo_url: e.target.value })} placeholder="رابط الشعار" className="rounded-xl bg-gray-800/50 p-4" /><input type="file" accept="image/*" onChange={e => e.target.files?.[0] && uploadImage(e.target.files[0], 'client')} className="rounded-xl bg-gray-800/50 p-3" />{uploading && <div className="text-sm text-green-300">جارٍ الرفع...</div>}<div className="grid grid-cols-2 gap-3"><button type="submit" className="rounded-xl bg-green-600 p-4">{editingClient ? 'تحديث' : 'إضافة'}</button><button type="button" onClick={resetForms} className="rounded-xl bg-gray-700 p-4">إغلاق</button></div></form>}
            </div>
          )}
